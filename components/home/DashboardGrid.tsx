      ...
      <div className="lg:col-span-8 flex flex-col gap-6">
        ...
        <section className="bg-white rounded-2xl shadow-sm border p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { title: '역삼초등학교', desc: '우리 학교에 대해 더 자세히 알아보아요', img: 'https://images.unsplash.com/photo-1594322436404-5a0526db4d13?w=500&q=80', badge: '학교', rating: '4.8', visits: 128 },
              { title: '선릉과 정릉', desc: '조선 왕릉의 역사와 문화를 배워요', img: 'https://images.unsplash.com/photo-1518780664697-55e3ad937233?w=500&q=80', badge: '명소', rating: '4.9', visits: 256 },
              { title: '역삼동 성당', desc: '근대 건축의 아름다움을 느껴보세요', img: 'https://images.unsplash.com/photo-1548625361-ec8538260b45?w=500&q=80', badge: '역사', rating: '4.7', visits: 89 },
            ].map((item, i) => (
              <div key={i} className="border rounded-xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
                <div className="h-32 bg-gray-200 relative">
                  ...
                  <span className="absolute bottom-2 left-2 bg-white/90 text-xs font-bold px-2 py-1 rounded">{item.badge}</span>
                </div>
                <div className="p-4">
                  <h3 className="font-bold mb-1 truncate">{item.title}</h3>
                  <p className="text-xs text-gray-500 mb-3 h-8 overflow-hidden">{item.desc}</p>
                  <div className="flex items-center gap-2 text-xs text-gray-400 font-medium">
                    <span className="text-yellow-500">★ {item.rating}</span>
                    <span>탐방 {item.visits}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <section className="bg-white rounded-2xl shadow-sm border p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-xs text-gray-500 hover:text-black">상세 보기 &gt;</button>
            </div>
            <div className="flex items-center gap-6 mb-6">
              <div className="w-20 h-20 rounded-full border-4 border-primary/20 flex items-center justify-center relative">
                ...
                <svg className="absolute inset-0 w-full h-full -rotate-90">
                  <circle cx="38" cy="38" r="36" fill="none" stroke="currentColor" strokeWidth="4" className="text-primary" strokeDasharray="226" strokeDashoffset="56" />
                </svg>
              </div>
              <div>
                <div className="font-bold text-lg">Lv. 3 탐험가</div>
                <div className="text-sm text-gray-500">다음 레벨까지 250 XP</div>
              </div>
            </div>
            <div className="grid grid-cols-4 text-center divide-x">
              <div><div className="text-xs text-gray-500 mb-1">탐방 완료</div><div className="font-bold">24</div></div>
              <div><div className="text-xs text-gray-500 mb-1">리뷰 작성</div><div className="font-bold">18</div></div>
              <div><div className="text-xs text-gray-500 mb-1">학습 자료</div><div className="font-bold">12</div></div>
              <div><div className="text-xs text-gray-500 mb-1">뱃지 획득</div><div className="font-bold">8</div></div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl shadow-sm border p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-xs text-gray-500 hover:text-black">더보기</button>
            </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-3 bg-white p-2 rounded-lg border shadow-sm">
                ...
                <button className="text-xs text-gray-500 px-2 py-1 border rounded hover:bg-gray-50">보기</button>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-white rounded-2xl shadow-sm border p-6">
          ...
          <div className="space-y-4">
            {[
              { name: '김학생', action: '역삼초등학교를 탐방했어요', time: '2시간 전' },
              { name: '이학생', action: '선릉과 정릉을 탐방했어요', time: '3시간 전' },
              { name: '박학생', action: '학습 자료를 생성했어요', time: '5시간 전' },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-bold">{item.name[0]}</div>
                <div className="flex-1 text-sm">
                  <span className="font-bold">{item.name}</span>님이 {item.action}
                </div>
        ...
        <section className="bg-[#0F172A] text-white rounded-2xl shadow-sm p-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="text-xs text-gray-400 hover:text-white">더보기</button>
            </div>
